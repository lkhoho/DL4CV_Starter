...
def convolve(image, K):
    ...
    # the input image so the spatial size (i.e., width and height) are not reduced
    pad = (kW - 1) // 2
    ...
    for y in np.arange(pad, iH + pad):
        for x in np.arange(pad, iW + pad):
            ...
            # multiplication between the ROI and the kernel, then summing the
            # matrix
            k = (roi * K).sum()
...
# construct the Laplacian kernel used to detect edge-like regions of the image
laplacian = np.array((
    [0, 1, 0],
    [1, -4, 1],
    [0, 1, 0]), dtype='int')

# construct the Sobel kernel
sobelX = np.array((
    [-1, 0, 1],
    [-2, 0, 2],
    [-1, 0, 1]), dtype='int')
